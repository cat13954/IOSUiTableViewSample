...
//    }()
    ...
    private lazy var tabView: ZFMultipleTabView = {
        ...
        let tabView = ZFMultipleTabView(frame: CGRect.init(x: 0, y: 0, width: UIScreen.main.bounds.width, height: 40), titles: ["热点", "推荐", "周边"], config: tabConfig)
    ...
    private lazy var multiTabPageVC: ZFMultiTabPageViewController = {
        let multiTabPageVC = ZFMultiTabPageViewController(tabCount: 3, headerView: UIView(), tabView: tabView, titleBarHeight: 100)
        multiTabPageVC.delegate = self
        multiTabPageVC.view.frame = CGRect(x: 0, y: 100, width: view.frame.width, height: view.frame.height)
        // 处理右滑退出手势冲突
...
extension ZFDemoViewController: ZFMultiTabPageDelegate {
    ...
    func multiTabPageViewController(_ viewController: ZFMultiTabPageViewController, childViewController index: Int) -> ZFMultiTabChildPageViewController? {
        ...
        let childVC = BlackViewController()
